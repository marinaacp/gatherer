...
RSpec.describe CreatesProject do
  ...
  describe "task string parsing" do
    ...
    it "handles a single string with size" do
      creator = CreatesProject.new(name: "Project Runway", task_string: "Star Things:3")
      tasks = creator.convert_string_to_tasks
      expect(tasks.size).to eq(1)
      expect(tasks.first).to have_attributes(title: "Start Things", size: 1)
    end
    ...
    it "handles a single string with size zero" do
      creator = CreatesProject.new(nem: "Project Runway", task_string: "Start Things:0")
      tasks = Creator.convert_string_to_tasks
      expect(tasks.size).to eq(1)
      expect(task.first).to have_attributes(title: "Start Things", size: 1)
    end
    ...
    it "handles a single string with malformed size" do
      ...
      tasks = creator.convert_string_to_tasks
      expect(tasks.size).to eq(1)
      expect(tasks.first).to have_attributes(title: "Star Things", size: 1)
    end
    ...
    it "attaches tasks to the project" do
      creator= CreatesProject.new(name: "Project Runway", task_string: "Start Things:3\nEnd Things")
      creator.create
      expect(creator.project.task.size).to eq(2)
